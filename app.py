...
st.markdown("""
    <style>
        ...
        /* Hide default Streamlit audio player visually but keep functional */
...
# --- 2. LOGIC & DATABASE (STRICT TRANSACTION CONTROL) ---
DB_PATH = "scores.db"
...
with tab_exam:
    ...
    if not available_words.empty:
        ...
        # Audio System (Hidden player, button-triggered)
        ...
        if st.session_state.play_trigger:
            ...
            st.session_state.play_trigger = False # Reset for next click
...
with tab_stats:
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if not bad_df.empty:
            ...
            st.success("No mistakes yet! Keep going, Vivian!")
